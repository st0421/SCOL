...
def project(
        G, 
        targetimg: torch.Tensor,  # [C,H,W] and dynamic range [0,255], W & H must match G output resolution
        *,
        num_steps=1000,
        w_avg_samples=10000,
    ...
    assert targetimg.shape == (G.img_channels, G.img_resolution, G.img_resolution)
    def logprint(*args):
        if verbose:
            print(*args)
    ...
    targetimg = targetimg.unsqueeze(0).to(device).to(torch.float32)
    
    # Features for target image.
    if targetimg.shape[2] > 256:
        targetimg = F.interpolate(targetimg, size=(256, 256), mode='area')
        
    target_features = vgg16(targetimg, resize_images=False, return_lpips=True)
    ...
    for step in tqdm(range(num_steps)):
        ...
        if use_wandb:
            wandb.log({f'reg_loss_{w_name}': reg_loss.detach().cpu()}, step=global_config.training_step)
            wandb.log({f'dist_{w_name}': dist.detach().cpu()}, step=global_config.training_step)
